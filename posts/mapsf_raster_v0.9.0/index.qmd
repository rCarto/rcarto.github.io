...
`mapsf` focuses on creating thematic maps based on vector data (maps are generated from `sf` objects). However, it is also possible to map raster data thanks to the `mf_raster()` function. 

The latest update of `mapsf` (v0.9.0) expands the mapping capabilities for raster data. Emulating the functionality of `terra` (and leveraging its plotting function internally), `mf_raster()` now provides three types of maps: "classes," "continuous," and "interval."

## Classes

This type of raster map is suitable for rasters containing qualitative data (categories).   
...
# map the raster
mf_raster(clc, pal = clc_pal, expandBB = c(0, 0.5, 0.01, 0),
          leg_title = "", leg_size = .7, leg_box_cex = c(.6, .85), leg_pos = "topleft")
...
This type of map is appropriate for numeric rasters.   
In the following example we display elevation maps of the Lot region.
...
# add annotations
max_elev_coord <- xyFromCell(elev, where.max(elev)[2])[1,]
...
This type of map is also appropriate for numeric rasters. The numeric values are classified using one the methods provided by `mf_get_breaks()`.
...
The last kind of map that `mf_raster()` can take care of is color composite maps using three bands. 
